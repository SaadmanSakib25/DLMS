...
def BookDetailView(request, pk):
    ...
    # reviews=Review.objects.filter(book=book).exclude(review="none")
    # try:
    #     stu = Student.objects.get(roll_no=request.user)
    #     rr=Review.objects.get(review="none")
    # except:
    #     pass
...
def BookCreate(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect(index)
    return render(request, 'catalog/book_form.html', locals())
...
def ret(request, pk):
    ...
    rating = Review(review="none", book=book,student=student,rating='2.5')
    rating.save()
...
def search_book(request):
    ...
    if ('q' in request.GET) and request.GET['q'].strip():
        ...
        entry_query = get_query(query_string, ['title', 'summary','author_1', 'author_2', 'author_3', 'isbn'])
